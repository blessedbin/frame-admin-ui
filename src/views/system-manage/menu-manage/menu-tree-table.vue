      ...
      <el-row :gutter="20">
        ...
        <el-col :span="16">
          <el-card class="box-card" v-loading="details.loading">
            <div slot="header" class="clearfix">
              <span>功能点</span>
              <el-button style="float: right; padding: 3px 0" type="text" @click="handleAddAction">添加功能点</el-button>
            </div>
            <div>
              <div v-if="details.data.operationDetails == null || details.data.operationDetails.length === 0">
                <span>无功能点</span>
              </div>
              <el-collapse v-model="details.activeNames" v-else>
                <el-collapse-item :title="operation.name" :name="operation.id" v-for="operation in details.data.operationDetails" :key="operation.id">
                  <div>
                    <el-button type="primary" @click="showEditApiDialog(operation.id)" size="mini" plain>编辑API</el-button>
                    <el-button type="danger" size="mini" plain>删除功能点</el-button>
                  </div>
                  <div v-if="operation.apiDetails != null && operation.apiDetails.length > 0" style="margin-top: 20px;">
                    <el-tag v-for="api in operation.apiDetails" :type="apiTagType(api)" style="margin: 5px;" :key="api.id">
                      {{ api.name }}:{{ api.url }}[{{ api.method }}]
                    </el-tag>
                  </div>
                  <div v-else style="margin-top: 20px;">
                    <span><el-tag>无API</el-tag></span>
                  </div>
                </el-collapse-item>
              </el-collapse>
              <!--<hr>
              <pre>{{details.data}}</pre>
              <hr>-->
            </div>
          </el-card>
        </el-col>
